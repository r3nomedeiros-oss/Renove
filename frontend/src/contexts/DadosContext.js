...
export const DadosProvider = ({ children }) => {
  ...
  const carregarLancamentos = useCallback(async (forceRefresh = false, filtros = {}) => {
    const temFiltros = filtros.dataInicio || filtros.dataFim;
  ...
  const carregarStatsMensal = useCallback(async (forceRefresh = false, periodo = 'mensal', dataInicio = '', dataFim = '') => {
    // Se é período customizado, busca direto
    if (periodo === 'customizado' && dataInicio && dataFim) {
      setLoadingStats(true);
      try {
        const response = await axios.get(
          `${API_URL}/relatorios?periodo=customizado&data_inicio=${dataInicio}&data_fim=${dataFim}&t=${Date.now()}`
        );
        return response.data;
      } catch (error) {
        console.error('Erro ao carregar stats customizado:', error);
        return null;
      } finally {
    ...
    try {
      ...
      const periodoData = await encontrarUltimoPeriodoComDados();

      // Buscar relatório para esse período específico
      const response = await axios.get(
        `${API_URL}/relatorios?periodo=customizado&data_inicio=${periodoData.data_inicio}&data_fim=${periodoData.data_fim}&t=${Date.now()}`
      );
      ...
      const statsComPeriodo = {
        ...
        periodo_referencia: {
          ano: periodoData.ano,
          mes: periodoData.mes,
          mesNome: new Date(periodoData.ano, periodoData.mes - 1, 1).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })
        }
      };
...
export function useDadosLegacy() {
  return useDados();
}
